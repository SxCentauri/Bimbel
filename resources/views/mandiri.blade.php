...
<head>
    ...
    <style>
        ...
        .btn-back {
            ...
            padding: 0.5rem 1rem; background-color: #ffc800; color: #fff;
            font-weight: bold; font-size: 0.875rem; border-radius: 0.5rem;
            text-decoration: none; transition: background-color 0.3s, transform 0.2s;
        }
        .btn-back:hover { background-color: #d9aa00; transform: scale(1.02); }
    </style>
</head>
<body id="page-top" class="bg-gray-50 text-gray-700 antialiased selection:bg-[#ffc800] selection:text-black flex flex-col min-h-screen">
    ...
    <nav class="fixed w-full z-50 top-0 transition-all duration-300 bg-gray-900 shadow-xl" id="mainNav">
            ...
            <div class="flex justify-between items-center h-20">
                ...
                <div class="hidden lg:flex items-center space-x-6">
                    @guest
                        ...
                        <a href="{{ route('login') }}" class="group relative inline-flex items-center gap-2 px-6 py-2 bg-[#ffc800] text-white font-bold rounded-full overflow-hidden shadow-lg transition-all duration-300 hover:scale-105 hover:shadow-[0_0_20px_rgba(255,200,0,0.5)] active:scale-95 text-sm">
                    ...
                    @else
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <a href="{{ route('password.edit') }}" 
                               class="group w-10 h-10 flex items-center justify-center rounded-full bg-gray-800 text-gray-400 hover:bg-gray-700 hover:text-[#ffc800] transition duration-300 border border-gray-700"
                               title="Ganti Password">
                                <i class="fas fa-key transition-transform group-hover:rotate-12"></i>
                            </a>
        ...
        <div id="mobile-menu" class="lg:hidden bg-gray-900 border-t border-gray-800 absolute w-full left-0 shadow-xl z-40">
            <ul class="px-6 py-6 space-y-4 font-bold text-white uppercase text-sm font-heading tracking-wide">
                ...
                @else
                    <li class="flex items-center gap-3 bg-gray-800 p-3 rounded-lg border border-gray-700">
                        <div class="w-10 h-10 rounded-full bg-brand flex items-center justify-center text-white font-bold text-lg shadow-md">
                            {{ substr(Auth::user()->name ?? 'U', 0, 1) }}
                        </div>
                        <div class="flex-col overflow-hidden">
                            <span class="block text-sm text-white truncate font-bold">{{ Auth::user()->name ?? 'User' }}</span>
                            <span class="block text-xs text-brand/80 font-medium">Siswa Aktif</span>
                        </div>
                    </li>

                    <li>
                        <a href="{{ route('password.edit') }}" 
                           class="flex items-center gap-3 p-3 rounded-lg hover:bg-gray-800 transition-colors duration-200 text-gray-300 hover:text-[#ffc800]">
                            <div class="w-8 h-8 rounded-full bg-gray-800 border border-gray-600 flex items-center justify-center">
                    ...
                    <li>
                        <form action="{{ route('logout') }}" method="POST">
                            @csrf
                            <button type="submit" class="w-full flex items-center justify-center gap-2 py-3 bg-red-600/90 text-white rounded-lg hover:bg-red-600 shadow-lg transition duration-200">
    ...
    <footer class="py-6 bg-gray-900 text-white text-center text-sm border-t border-gray-800 mt-auto">
        <div class="container mx-auto text-gray-400">
            &copy; {{ date('Y') }} ONMAI. All rights reserved.
        </div>
    </footer>
